import { getCombinedDemandForRoute, getOperatingCostPerTrip, getPassengersPerTrip, } from "./economy";
import { calculateRealFuelFromUnits, calculateRouteTripsPerWeek, } from "./trips";
function getVehicleTypeForRouteMode(mode) {
    switch (mode) {
        case "rail":
            return "train";
        case "air":
...
function getCityName(game, cityId) {
    return game.cities.find(city => city.id === cityId)?.name ?? cityId;
}
function getOwnedVehicleCards(game, player) {
...
function assignVehicleCardsToRoutes(game, player, ownedCards) {
    ...
    const explicitAssignmentsByRouteId = {};
    const usedCardIds = new Set();
    for (const route of ownedRoutes) {
        const assignedCardId = game.bureaucracyVehicleCardIdsByRouteId[route.id];
        const assignedCard = assignedCardId === undefined ? null : cardsById.get(assignedCardId) ?? null;
        if (assignedCard &&
    ...
    return ownedRoutes.map(route => {
        ...
        return {
            route,
            vehicleCard: assignedCard ?? cardsByType[getVehicleTypeForRouteMode(route.mode)].shift() ?? null,
        };
    });
}
export function buildPlayerBureaucracySummary(game, playerId) {
    const player = game.players.find(candidate => candidate.id === playerId);
    if (!player) {
        return null;
    }
    const ownedCards = getOwnedVehicleCards(game, player);
    const remainingFuelUnits = {
        diesel: player.inventory.fuel.diesel,
        jetFuel: player.inventory.fuel.jetFuel,
    };
    const routePlans = assignVehicleCardsToRoutes(game, player, ownedCards).map(({ route, vehicleCard }) => {
        const statsVehicleCard = vehicleCard ??
        ...
        const routeTripSummary = vehicleCard === null
            ? null
            : calculateRouteTripsPerWeek(game, route, vehicleCard);
        const statsRouteTripSummary = statsVehicleCard === null
            ? null
            : calculateRouteTripsPerWeek(game, route, statsVehicleCard);
        const maxFuelUnitsByTime = routeTripSummary && routeTripSummary.fuelResource !== null
        ...
        const maxFuelUnitsByInventory = routeTripSummary?.fuelResource
            ...
            : 0;
        const selectedFuelUnits = routeTripSummary === null || routeTripSummary.fuelResource === null
            ? 0
            : Math.min(requestedFuelUnits, maxFuelUnitsByTime, maxFuelUnitsByInventory);
        const selectedTrips = routeTripSummary === null
            ? 0
            : routeTripSummary.fuelResource === null
                ? routeTripSummary.tripsPerWeek
                : Math.min(routeTripSummary.tripsPerWeek, Math.floor((selectedFuelUnits + 1e-9) /
                    Math.max(routeTripSummary.tripFuelUnits, 0.000001)));
        const passengersPerTrip = statsVehicleCard === null ? 0 : getPassengersPerTrip(game, route, statsVehicleCard);
        const passengersServed = vehicleCard === null
            ? 0
            : selectedTrips * getPassengersPerTrip(game, route, vehicleCard);
        const revenue = (routeTripSummary?.distanceMiles ?? 0) *
            passengersServed *
            game.operatingConfig.revenuePerPassengerMile[route.mode];
        const totalFuelBurnReal = routeTripSummary?.fuelResource
            ? calculateRealFuelFromUnits(selectedFuelUnits, routeTripSummary.fuelResource, game)
            : 0;
        const operatingCost = vehicleCard === null ? 0 : selectedTrips * getOperatingCostPerTrip(game, route);
        if (routeTripSummary?.fuelResource) {
            remainingFuelUnits[routeTripSummary.fuelResource] = Math.max(0, remainingFuelUnits[routeTripSummary.fuelResource] - selectedFuelUnits);
        }
        return {
            route,
...
export function buildBureaucracySummaries(game) {
    return game.players
        .map(player => buildPlayerBureaucracySummary(game, player.id))
        .filter((summary) => summary !== null);
}
export function getMaxFuelUnitsForRoute(game, routeId) {
    ...
    const otherFuelUsage = summary.routePlans
        .filter(plan => plan.route.id !== routeId && plan.fuelResource === targetPlan.fuelResource)
        .reduce((total, plan) => total + plan.totalFuelBurnUnits, 0);
    const availableFuelUnits = Math.max(0, summary.player.inventory.fuel[targetPlan.fuelResource] - otherFuelUsage);
    return Math.max(0, Math.min(targetPlan.maxFuelUnitsByTime, Math.floor(availableFuelUnits)));
}
export function getMaxFuelUnitsCapacityForPlayer(game, playerId, resource) {
...
export function applyBureaucracyFuelConsumption(game) {
    ...
    return {
        ...
        players: game.players.map(player => {
            ...
            return {
                ...
                totalPassengersServed: player.totalPassengersServed + summary.totalPassengersServed,
                operatingCosts: summary.totalOperatingCost,
                weeklyPayout: summary.totalRevenue,
                inventory: {
                    ...player.inventory,
                    fuel: {
                        diesel: Math.max(0, player.inventory.fuel.diesel - summary.fuelUsedUnits.diesel),
                        jetFuel: Math.max(0, player.inventory.fuel.jetFuel - summary.fuelUsedUnits.jetFuel),
                    },
                },
            };
        }),
